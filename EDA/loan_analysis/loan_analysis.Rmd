...
cut_loan <- function(df) {
  filter(df, !is.na(LoanOriginalAmount))$LoanOriginalAmount %>%
    cut(
      c(0, 5001, 10001, 15001, 20001, 40000),
...
theme.set <-
  function(plot_type ,
    ...
    if (missing(x_element_text)) {
      x_element_text = list(face = "bold",
                            size = 6,
                            angle = 90)
    }
    if (missing(y_element_text)) {
      y_element_text = list(face = "bold",
                            size = 6,
                            angle = 0)
    }
    if (missing(guide)) {
    ...
    pl <- function()
      ...
      ylab(yl) +
      theme(
        axis.text.x = do.call(element_text, x_element_text),
...
loan_df <-
  ...
  IncomeRange != "$0" &
  !is.na(IncomeRange) &
  IncomeRange != "Not employed" & IncomeRange != "Not displayed"
  )
...
levels(loan_df$LoanStatus) <- c(levels(loan_df$LoanStatus),
                                ...
                                loan_df$LoanStatus[loan_df$LoanStatus %in% c(
                                "Past Due (31-60 days)",
                                "Past Due (61-90 days)",
...
```{r echo=FALSE, fig.retina=2, out.height="100%", out.width="100%"}
p_mean_by_state <-
  ggplot(data = mean_by_states, aes(BorrowerState, y = freq * 100)) +
  geom_point(
    colour = "white",
    shape = 21,
    size = 2,
  ...
  stat_summary(fun.y = "mean",
               geom = "line",
               aes(group = LoanStatus, color = LoanStatus))
...
p_states_count <-
  ggplot(data = states_count, aes(BorrowerState, y = freq)) +
...
sub_fail <-
  subset(loan_df, LoanStatus == "Defaulted" |
           LoanStatus == "Chargedoff")

p_failed_by_incomerange <- ggplot(data = sub_fail,
                                  aes(
...
box_in1 <-
  ggplot(
    data = subset(loan_df, LoanStatus == "Defaulted" |
                    LoanStatus == "Completed"),
    aes(x = 1, y = LoanOriginalAmount)
...
x_gr <-
  group_by(sub_fail, IncomeRange, LoanAmountShort, LoanStatus) %>% 
  summarise(n = n(), LOA = mean(LoanOriginalAmount))
N <- sum(x_gr$n)
p_violin <-
  ggplot(data = x_gr, aes(y = n, x = LoanAmountShort, fill = LoanStatus)) +
  geom_violin(scale = "width")
p_violin_final <-
  theme.set(
    p_violin + ggtitle("Customers per loan bucket"),
    "Loan bucket",
    "Customers in bucket",
    x_element_text = list(face = "bold", size =
                            9, angle = 90),
    y_element_text = list(face = "bold", size =
                            9, angle = 0),
    legend_pos = "top"
  )
p_violin_final()
...
loan_df.sum <- loan_df %>%
  dplyr::select(LoanOriginalAmount,
                Term,
                MonthlyLoanPayment) %>%
  summarise_all(
    funs(
...
```{r echo=FALSE, out.width="100%", out.height="100%", fig.retina=2}
loan_amount <-
  filter(loan_df, !is.na(LoanOriginalAmount))$LoanOriginalAmount %>%
  cut(
    c(0, 2001, 4001, 6001, 10001, 15001, 20001, 40001),
...
ggplot(data = filter(loan_df, LoanStatus != "Current"),
...
p2 <- ggplot(
  data = filter(loan_df, LoanStatus != "Current"),
  aes(x = CreditScoreRangeLower, y = ..count.. / sum(..count..))
) +
  geom_histogram(aes(fill = LoanStatus), binwidth = 5)
...
failed_loans <-
  filter(loan_df, LoanStatus %in% c("Chargedoff", "Defaulted"))
...
p1_1 <-
  ggplot(data = failed_loans, aes(LoanStatus, MonthlyLoanPayment)) +
  geom_boxplot() + coord_cartesian(ylim = c(0, 700)) +
  facet_wrap( ~ IncomeRange, nrow = 1, scales = "free_y")
p2_2 <-
  ggplot(data = failed_loans, aes(LoanStatus, CurrentCreditLines)) +
  geom_boxplot() + coord_cartesian(ylim = c(0, 20)) +
  facet_wrap( ~ IncomeRange, nrow = 1, scales = "free_y")
p3_3 <-
  ggplot(data = failed_loans, aes(LoanStatus, LoanOriginalAmount)) +
...
p_score_split <-
  ggplot(data = x,
         aes(y = LoanOriginalAmount, x = CreditScoreRangeLower, color = LoanStatus)) +
  geom_point(size = .75,
             alpha = .7,
             position = 'jitter') +
  geom_line(stat = "summary",
...
```{r echo=FALSE, fig.retina=2, out.width="100%"}
fwrap_income <- filter(loan_df,
                       LoanStatus %in% c("Chargedoff",
                                         "Defaulted",
                                         "Completed",
                                         "Current"))
fw <- ggplot(data = fwrap_income,
             aes(y = LoanOriginalAmount,
                 x = CreditScoreRangeLower,
                 color = IncomeRange)) +
  geom_point(size = .75,
...
amount_gr <- group_by(loan_df,
  ...
  summarise(
    n = n(),
    CreditScoreMean = mean(CreditScoreRangeLower),
...
amount_gr_sb <-
  subset(amount_gr,
         LoanStatus %in% c("Defaulted", "Chargedoff", "Completed"))

p_loan_income_ranges <-
  ggplot(data = amount_gr_sb, aes(x = CreditScoreMean, y = freq)) +
  geom_line(aes(color = LoanStatus)) +
  facet_wrap( ~ LoanAmountShort) +
  geom_vline(
    data = amount_gr,
    xintercept = c(500, 590, 670, 750, 860, 900),
    linetype = 3
  ) +
  geom_point(
    colour = "white",
...
p_def <- ggplot(data = amount_gr_sb1, aes(CreditScoreShort, freq)) +
  geom_bar(stat = "identity",
           ...
           aes(fill = IncomeRange)) +
  facet_wrap( ~ LoanAmountShort) + scale_fill_brewer(palette = "Purples") +
  ggtitle("Chargedoff and Defaulted customers proportion 
        per loan bucket for each income range")
...
theme.set(
  p_mean_by_state + scale_y_log10() + ggtitle("Means of loans statuses over states"),
  "State",
  "Freq in %",
  list(face = "bold", size = 6, angle = 45),
  guide = list(keywidth = 0.7, keyheight = 0.7),
